trait_opt_bivar_start_KsTLP <- function(P50,
                      ...
                      LS_from_P50_TLP_Ks,
                      Ks_from_P50_LS_slope_WD,#Ks_from_P50_LS_slope,
                      TLP_from_LS_LMA_P50_slope,
                      ...
                      slope_from_WD,
                      Ks_from_LS,
                      LS_from_LMA, #WD from LMA this one is always initiated from data with B
                      Ks_e_start,
                      TLP_e_start,
                      n_uncer, 
                      use_LMA_from_TLP_LS) {
  ...
  # The random sampling comes from the bootstrap sampling done in the calculations of the regressions themselves. 
  # This approach has the big advantages of 
  ...
  if (ss==1) { #First pass always calculates the  best estimate
    if(use_LMA_from_TLP_LS) {   # BE:
      mod_LMA_intercept_sample <- LMA_from_TLP_LS_WD$mod$intercept_R #LMA_from_TLP_LS_WD
      mod_LMA_slope_y1_sample  <- LMA_from_TLP_LS_WD$mod$slope_R.y1
      mod_LMA_slope_y2_sample  <- LMA_from_TLP_LS_WD$mod$slope_R.y2
      mod_LMA_slope_y3_sample  <- LMA_from_TLP_LS_WD$mod$slope_R.y3
      
      mod_LS_intercept_sample  <- LS_from_P50_TLP_Ks_LMA$mod$intercept_R #LS_from_P50_TLP_Ks_LMA
      ...
      mod_WD_intercept_sample   <- WD_from_P50_slope_Ks_LMA$mod$intercept_R #WD_from_P50_slope_Ks# WD_from_Ks_P50#WD_from_slope_P50slope
      mod_WD_slope_y1_sample    <- WD_from_P50_slope_Ks_LMA$mod$slope_R.y1 
      mod_WD_slope_y2_sample    <- WD_from_P50_slope_Ks_LMA$mod$slope_R.y2 
      mod_WD_slope_y3_sample    <- WD_from_P50_slope_Ks_LMA$mod$slope_R.y3 
      mod_WD_slope_y4_sample    <- WD_from_P50_slope_Ks_LMA$mod$slope_R.y4 #WD_from_P50_slope_Ks_LMA

    } else {  # BDT:
      mod_LMA_intercept_sample <- LMA_from_TLP$mod$intercept_R #LMA_from_TLP
      mod_LMA_slope_y1_sample  <- LMA_from_TLP$mod$slope_R.y1
      
      mod_LS_intercept_sample  <- LS_from_P50_TLP_Ks$mod$intercept_R #LS_from_P50_TLP_Ks   
      mod_LS_slope_y1_sample   <- LS_from_P50_TLP_Ks$mod$slope_R.y1   
      mod_LS_slope_y2_sample   <- LS_from_P50_TLP_Ks$mod$slope_R.y2 
      mod_LS_slope_y3_sample   <- LS_from_P50_TLP_Ks$mod$slope_R.y3
      
      mod_WD_intercept_sample   <- WD_from_P50_slope_Ks$mod$intercept_R #WD_from_P50_slope_Ks# WD_from_Ks_P50#WD_from_slope_P50slope
      mod_WD_slope_y1_sample    <- WD_from_P50_slope_Ks$mod$slope_R.y1
      mod_WD_slope_y2_sample    <- WD_from_P50_slope_Ks$mod$slope_R.y2
      mod_WD_slope_y3_sample    <- WD_from_P50_slope_Ks$mod$slope_R.y3
    }
    mod_Ks_intercept_sample    <- Ks_from_P50_LS_slope_WD$mod$intercept #Ks_from_P50_LS
    mod_Ks_slope_y1_sample     <- Ks_from_P50_LS_slope_WD$mod$slope_R.y1
    mod_Ks_slope_y2_sample     <- Ks_from_P50_LS_slope_WD$mod$slope_R.y2
    mod_Ks_slope_y3_sample     <- Ks_from_P50_LS_slope_WD$mod$slope_R.y3
    mod_Ks_slope_y4_sample     <- Ks_from_P50_LS_slope_WD$mod$slope_R.y4
    mod_TLP_intercept_sample   <- TLP_from_LS_LMA_P50_slope$mod$intercept_R #TLP_from_LS_LMA_P50
    mod_TLP_slope_y1_sample    <- TLP_from_LS_LMA_P50_slope$mod$slope_R.y1
    mod_TLP_slope_y2_sample    <- TLP_from_LS_LMA_P50_slope$mod$slope_R.y2
    mod_TLP_slope_y3_sample    <- TLP_from_LS_LMA_P50_slope$mod$slope_R.y3
    mod_TLP_slope_y4_sample    <- TLP_from_LS_LMA_P50_slope$mod$slope_R.y4
    mod_P50_intercept_sample   <- P50_from_TLP_LS_Ks_slope_WD$mod$intercept_R #P50_from_TLP_Ks
    mod_P50_slope_y1_sample    <- P50_from_TLP_LS_Ks_slope_WD$mod$slope_R.y1
    mod_P50_slope_y2_sample    <- P50_from_TLP_LS_Ks_slope_WD$mod$slope_R.y2
    mod_P50_slope_y3_sample    <- P50_from_TLP_LS_Ks_slope_WD$mod$slope_R.y3
    mod_P50_slope_y4_sample    <- P50_from_TLP_LS_Ks_slope_WD$mod$slope_R.y4
    mod_P50_slope_y5_sample    <- P50_from_TLP_LS_Ks_slope_WD$mod$slope_R.y5
    ...
    if(use_LMA_from_TLP_LS) {
      mod_LMA_intercept_sample <- LMA_from_TLP_LS_WD$mod$boot.intercept[ss] #LMA_from_TLP_LS
      mod_LMA_slope_y1_sample  <- LMA_from_TLP_LS_WD$mod$boot.y1[ss]
      mod_LMA_slope_y2_sample  <- LMA_from_TLP_LS_WD$mod$boot.y2[ss]
      mod_LMA_slope_y3_sample  <- LMA_from_TLP_LS_WD$mod$boot.y3[ss]
      
      mod_LS_intercept_sample  <- LS_from_P50_TLP_Ks_LMA$mod$boot.intercept[ss] #LS_from_LMA TLP_Ks
      mod_LS_slope_y1_sample   <- LS_from_P50_TLP_Ks_LMA$mod$boot.y1[ss] 
      mod_LS_slope_y2_sample   <- LS_from_P50_TLP_Ks_LMA$mod$boot.y2[ss] 
      mod_LS_slope_y3_sample   <- LS_from_P50_TLP_Ks_LMA$mod$boot.y3[ss] 
      mod_LS_slope_y4_sample   <- LS_from_P50_TLP_Ks_LMA$mod$boot.y4[ss] 
      
      mod_WD_intercept_sample   <- WD_from_P50_slope_Ks_LMA$mod$boot.intercept[ss] 
      mod_WD_slope_y1_sample    <- WD_from_P50_slope_Ks_LMA$mod$boot.y1[ss]# WD_from_P50_slope_Ks_LMA
      mod_WD_slope_y2_sample    <- WD_from_P50_slope_Ks_LMA$mod$boot.y2[ss]
      mod_WD_slope_y3_sample    <- WD_from_P50_slope_Ks_LMA$mod$boot.y3[ss]
      mod_WD_slope_y4_sample    <- WD_from_P50_slope_Ks_LMA$mod$boot.y4[ss]
    } else {
      ...
      mod_LS_slope_y3_sample   <- LS_from_P50_TLP_Ks$mod$boot.y3[ss] 
      
      mod_WD_intercept_sample   <- WD_from_P50_slope_Ks$mod$boot.intercept[ss] # WD_from_P50_slope_Ks#WD_from_slope_P50slope
      mod_WD_slope_y1_sample    <-  WD_from_P50_slope_Ks$mod$boot.y1[ss]
      mod_WD_slope_y2_sample    <- WD_from_P50_slope_Ks$mod$boot.y2[ss]
      mod_WD_slope_y3_sample    <- WD_from_P50_slope_Ks$mod$boot.y3[ss]
    }
    mod_Ks_intercept_sample <- Ks_from_P50_LS_slope_WD$mod$boot.intercept[ss] #Ks_from_P50_LS
    mod_Ks_slope_y1_sample <- Ks_from_P50_LS_slope_WD$mod$boot.y1[ss]
    mod_Ks_slope_y2_sample <- Ks_from_P50_LS_slope_WD$mod$boot.y2[ss]
    mod_Ks_slope_y3_sample <- Ks_from_P50_LS_slope_WD$mod$boot.y3[ss]
    mod_Ks_slope_y4_sample <- Ks_from_P50_LS_slope_WD$mod$boot.y4[ss]
    mod_TLP_intercept_sample <- TLP_from_LS_LMA_P50_slope$mod$boot.intercept[ss] #TLP_from_LS_LMA_P50
    mod_TLP_slope_y1_sample <- TLP_from_LS_LMA_P50_slope$mod$boot.y1[ss]
    mod_TLP_slope_y2_sample <- TLP_from_LS_LMA_P50_slope$mod$boot.y2[ss]
    mod_TLP_slope_y3_sample <- TLP_from_LS_LMA_P50_slope$mod$boot.y3[ss]
    mod_TLP_slope_y4_sample <- TLP_from_LS_LMA_P50_slope$mod$boot.y4[ss]
    
    mod_P50_intercept_sample <- P50_from_TLP_LS_Ks_slope_WD$mod$boot.intercept[ss] #P50_from_TLP_Ks
    mod_P50_slope_y1_sample <- P50_from_TLP_LS_Ks_slope_WD$mod$boot.y1[ss]
    mod_P50_slope_y2_sample <- P50_from_TLP_LS_Ks_slope_WD$mod$boot.y2[ss]
    mod_P50_slope_y3_sample <- P50_from_TLP_LS_Ks_slope_WD$mod$boot.y3[ss]
    mod_P50_slope_y4_sample <- P50_from_TLP_LS_Ks_slope_WD$mod$boot.y4[ss]
    mod_P50_slope_y5_sample <- P50_from_TLP_LS_Ks_slope_WD$mod$boot.y5[ss]
    ...
    LMA_e_last = LMA_from_TLP$mod$intercept_R + LMA_from_TLP$mod$slope_R.y1*TLP_e_start
    ...
    LS_e_last = LS_from_LMA$mod$intercept_R  + LS_from_LMA$mod$slope_R.y1*LMA_e_last
    ...
    # Do the first iteration with the starting trait pair ('_start'). In the optimisation below, these traits also turn into predicted variables.
    if(use_LMA_from_TLP_LS) {
      LMA_e_last = mod_LMA_intercept_sample +  mod_LMA_slope_y1_sample*TLP_e_start +
        mod_LMA_slope_y2_sample*LS_e_last + mod_LMA_slope_y3_sample * WD_e_last
      
      LS_e_last = mod_LS_intercept_sample + mod_LS_slope_y1_sample * P50_e_last +  mod_LS_slope_y2_sample*TLP_e_start +
      mod_LS_slope_y3_sample * Ks_e_start + mod_LS_slope_y4_sample * LMA_e_last        
      
      WD_e_last = mod_WD_intercept_sample  + mod_WD_slope_y1_sample*P50_e_last  + mod_WD_slope_y2_sample*slope_e_last+ 
        mod_WD_slope_y3_sample*Ks_e_start  + mod_WD_slope_y4_sample*LMA_e_last 
    } else {
      LMA_e_last = mod_LMA_intercept_sample + mod_LMA_slope_y1_sample*TLP_e_start
      ...
      WD_e_last = mod_WD_intercept_sample  + mod_WD_slope_y1_sample*P50_e_last  + mod_WD_slope_y2_sample*slope_e_last+ 
        mod_WD_slope_y3_sample*Ks_e_start
    }
    
   # Ks_e_last  = mod_Ks_intercept_sample + mod_Ks_slope_y1_sample*P50_e_last +  
   #   mod_Ks_slope_y2_sample* TLP_e_last  +  mod_Ks_slope_y3_sample* slope_e_last  +  mod_Ks_slope_y4_sample* WD_e_last
    
   # TLP_e_last = mod_TLP_intercept_sample + mod_TLP_slope_y1_sample*LS_e_last + 
   #   mod_TLP_slope_y2_sample*LMA_e_last + mod_TLP_slope_y3_sample*P50_e_last + mod_TLP_slope_y4_sample*slope_e_last
    
     P50_e_last = mod_P50_intercept_sample + mod_P50_slope_y1_sample*TLP_e_start +  mod_P50_slope_y2_sample*LS_e_last + 
    ...
    while (T) {
      ...
      if(use_LMA_from_TLP_LS) {
        LMA_e[ss]= mod_LMA_intercept_sample + mod_LMA_slope_y1_sample*TLP_e_start +
          mod_LMA_slope_y2_sample*LS_e_last + mod_LMA_slope_y3_sample * WD_e_last
        
        LS_e[ss] = mod_LS_intercept_sample + mod_LS_slope_y1_sample * P50_e_last +  mod_LS_slope_y2_sample*TLP_e_start +
             mod_LS_slope_y3_sample * Ks_e_start + mod_LS_slope_y4_sample * LMA_e_last       
        
        WD_e[ss] = mod_WD_intercept_sample  + mod_WD_slope_y1_sample*P50_e_last  + mod_WD_slope_y2_sample*slope_e_last+ 
          mod_WD_slope_y3_sample*Ks_e_start  + mod_WD_slope_y4_sample*LMA_e_last 
     ...
     # TLP_e[ss] = mod_TLP_intercept_sample + mod_TLP_slope_y1_sample*LS_e_last + 
     #   mod_TLP_slope_y2_sample*LMA_e_last + mod_TLP_slope_y3_sample*P50_e_last + mod_TLP_slope_y4_sample*slope_e_last
      
        P50_e[ss] = mod_P50_intercept_sample + mod_P50_slope_y1_sample*TLP_e_start +  mod_P50_slope_y2_sample*LS_e_last + 
         mod_P50_slope_y3_sample*Ks_e_start + mod_P50_slope_y4_sample*slope_e_last + mod_P50_slope_y5_sample*WD_e_last
     ...
     #   mod_Ks_slope_y3_sample* slope_e_last  +  mod_Ks_slope_y4_sample* WD_e_last
      ...
      if (limitdataranges) {
       ...
       # if (TLP_e[ss]<minTLP | is.na(TLP_e[ss])) {TLP_e[ss]=NA; break}
        ...
        if (LS_e[ss]>maxLS | is.na(LS_e[ss])) {LS_e[ss]=NA; print('wrong');break}
        if (LS_e[ss]<minLS | is.na(LS_e[ss])) {LS_e[ss]=NA; print('wrong'); break}
  ...
  return_vals <- list("P50_e"=P50_e,"TLP_e"=TLP_e_start,"LMA_e"=LMA_e,"WD_e"=WD_e,"slope_e"=slope_e,'LS_e'= LS_e,'Ks_e'=Ks_e_start)
