trait_opt_bivar_start_KsTLP <- function(P50,
  ...
  for (ss in 1:n_uncer) {
    ...
    #  mod_Ks_slope_y3_sample     <- Ks_from_P50_LS_WD$mod$slope_R.y3
      ...
      if(use_LMA_from_TLP_LS_WD) { # BE:
        mod_LMA_intercept_sample <- LMA_from_TLP_LS_WD$mod$boot.intercept[ss] #LMA_from_TLP_LS_WD
        ...
        mod_LS_intercept_sample  <- LS_from_P50_TLP_Ks$mod$boot.intercept[ss] #LS_from_P50_TLP_Ks
        mod_LS_slope_y1_sample   <- LS_from_P50_TLP_Ks$mod$boot.y1[ss] 
        mod_LS_slope_y2_sample   <- LS_from_P50_TLP_Ks$mod$boot.y2[ss] 
        mod_LS_slope_y3_sample   <- LS_from_P50_TLP_Ks$mod$boot.y2[ss] 
      ...
      # now  BE or BDT -specific:
      #mod_WD_intercept_sample   <- WD_from_P50_Ks$mod$boot.intercept[ss] # WD_from_P50_slope_Ks#WD_from_slope_P50slope
      #mod_WD_slope_y1_sample    <- WD_from_P50_Ks$mod$boot.y1[ss]
      #mod_WD_slope_y2_sample    <- WD_from_P50_Ks$mod$boot.y2[ss]
      
    #  mod_Ks_intercept_sample    <- Ks_from_P50_LS_WD$mod$boot.intercept #Ks_from_P50_LS_WD
    ...
    #  mod_Ks_slope_y3_sample     <- Ks_from_P50_LS_WD$mod$boot.y3[ss]
      ...
      mod_slope_intercept_sample <- slope_from_P50_TLP_WD_Ks$mod$boot.intercept[ss] #slope_from_P50_TLP_WD_Ks
    ...
    while (T) {
      ...
      if(use_LMA_from_TLP_LS_WD) { #BE:
        ...
        if(regr_type=='pcr' || regr_type=='plsr'){
          TLP_e_start <- scale_traits(TLP_e_start, "TLP_e_start", nlabels, traits_mean, traits_sd)
          LS_e_last <- scale_traits(LS_e_last, "LS_e_last", nlabels, traits_mean, traits_sd)
          WD_e_last  <- scale_traits(WD_e_last, "WD_e_last", nlabels, traits_mean, traits_sd)
        ...
        #unscale
        if(regr_type=='pcr' || regr_type=='plsr'){
          TLP_e_start <- unscale_traits(TLP_e_start, "TLP_e_start", nlabels, traits_mean, traits_sd)
          LS_e_last   <- unscale_traits(LS_e_last, "LS_e_last", nlabels, traits_mean, traits_sd)
          WD_e_last   <- unscale_traits(WD_e_last, "WD_e_last", nlabels, traits_mean, traits_sd)
        ...
        #LS_from_P50_TLP_Ks_LMA
        if(regr_type=='pcr' || regr_type=='plsr'){
          P50_e_last  <- scale_traits(P50_e_last, "P50_e_last", nlabels, traits_mean, traits_sd)
          TLP_e_start <- scale_traits(TLP_e_start, "TLP_e_start", nlabels, traits_mean, traits_sd)
          Ks_e_start  <- scale_traits(Ks_e_start, "Ks_e_start", nlabels, traits_mean, traits_sd)
          LMA_e[ss]   <- scale_traits(LMA_e[ss], "LMA_e", nlabels, traits_mean, traits_sd)
        }
         LS_e[ss] = mod_LS_intercept_sample + mod_LS_slope_y1_sample*P50_e_last +  mod_LS_slope_y2_sample*TLP_e_start +
           mod_LS_slope_y3_sample * Ks_e_start + mod_LS_slope_y4_sample * LMA_e[ss]
        
        #unscale
        if(regr_type=='pcr' || regr_type=='plsr'){
          P50_e_last   <- unscale_traits(P50_e_last, "P50_e_last", nlabels, traits_mean, traits_sd)
          TLP_e_start  <- unscale_traits(TLP_e_start, "TLP_e_start", nlabels, traits_mean, traits_sd)
          Ks_e_start   <- unscale_traits(Ks_e_start, "Ks_e_start", nlabels, traits_mean, traits_sd)
          LMA_e[ss]    <- unscale_traits(LMA_e[ss], "LMA_e", nlabels, traits_mean, traits_sd)
        }
        ########
        ...
        WD_e[ss] = mod_WD_intercept_sample  + mod_WD_slope_y1_sample*P50_e_last  + 
          mod_WD_slope_y2_sample*LMA_e[ss]  +  mod_WD_slope_y3_sample*Ks_e_start
        ...
        WD_e[ss] = mod_WD_intercept_sample  + mod_WD_slope_y1_sample*P50_e_last  + 
          mod_WD_slope_y2_sample*Ks_e_start
     ...
     #   mod_TLP_slope_y2_sample*LMA_e_last + mod_TLP_slope_y3_sample*P50_e_last + mod_TLP_slope_y4_sample*slope_e_last
      ...
      if(regr_type=='pcr' || regr_type=='plsr'){
        ...
        P50_e[ss] = mod_P50_intercept_sample + mod_P50_slope_y1_sample*TLP_e_start +  
          mod_P50_slope_y2_sample*Ks_e_start  
    ...
    #scale
    if(regr_type=='pcr' || regr_type=='plsr'){
      P50_e[ss] <- scale_traits(P50_e[ss], "P50_e_last", nlabels, traits_mean, traits_sd)
      TLP_e_start   <- scale_traits(TLP_e_start, "TLP_e_start", nlabels, traits_mean, traits_sd)
    ...
    slope_e[ss] = mod_slope_intercept_sample + mod_slope_slope_y1_sample*P50_e[ss] + 
      mod_slope_slope_y2_sample*TLP_e_start  + mod_slope_slope_y3_sample*WD_e[ss] +
      mod_slope_slope_y4_sample*Ks_e_start
    
    #unscale
    if(regr_type=='pcr' || regr_type=='plsr'){
      P50_e[ss] <- unscale_traits(P50_e[ss], "P50_e_last", nlabels, traits_mean, traits_sd)
      TLP_e_start   <- unscale_traits(TLP_e_start, "TLP_e_start", nlabels, traits_mean, traits_sd)
      WD_e[ss]    <- unscale_traits(WD_e[ss], "WD_e", nlabels, traits_mean, traits_sd)
      Ks_e_start    <- unscale_traits(Ks_e_start, "Ks_e", nlabels, traits_mean, traits_sd)
