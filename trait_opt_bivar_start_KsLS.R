trait_opt_bivar_start_KsLS <- function(P50,
                      TLP,
                      LMA,
                      WD,
                      slope,
   ...
   # Ks_e_last = Ks_from_LS$mod$intercept_R  + Ks_from_LS$mod$slope_R.y1*LS_e_start
    ...
    while (T) {
      ...
      if(regr_type=='pcr' || regr_type=='plsr'){
        P50_e_last <- scale_traits(P50_e_last, "P50_e_last", nlabels, traits_mean, traits_sd)
        LS_e_start <- scale_traits(LS_e_start, "LS_e_start", nlabels, traits_mean, traits_sd)
        LMA_e[ss] <- scale_traits(LMA_e[ss], "LMA_e_last", nlabels, traits_mean, traits_sd)
      }
      
      TLP_e[ss] = mod_TLP_intercept_sample + mod_TLP_slope_y1_sample*LS_e_start + 
        mod_TLP_slope_y2_sample*LMA_e[ss] + mod_TLP_slope_y3_sample*P50_e_last 
      
      #unscale
      if(regr_type=='pcr' || regr_type=='plsr'){
        P50_e_last <- unscale_traits(P50_e_last, "P50_e_last", nlabels, traits_mean, traits_sd)
        LS_e_start <- unscale_traits(LS_e_start, "LS_e_start", nlabels, traits_mean, traits_sd)
        LMA_e[ss] <- unscale_traits(LMA_e[ss], "LMA_e_last", nlabels, traits_mean, traits_sd)
      ...
      #scale
      if(regr_type=='pcr' || regr_type=='plsr'){
        TLP_e[ss] <- scale_traits(TLP_e[ss], "TLP_e_last", nlabels, traits_mean, traits_sd)
        Ks_e_start <- scale_traits(Ks_e_start, "Ks_e_start", nlabels, traits_mean, traits_sd)
          }
      
      P50_e[ss] = mod_P50_intercept_sample + mod_P50_slope_y1_sample*TLP_e[ss] +  
        mod_P50_slope_y2_sample*Ks_e_start  
      
      #unscale
      if(regr_type=='pcr' || regr_type=='plsr'){
        TLP_e[ss]  <- unscale_traits(TLP_e[ss], "TLP_e_last", nlabels, traits_mean, traits_sd)
        Ks_e_start <- unscale_traits(Ks_e_start, "Ks_e_start", nlabels, traits_mean, traits_sd)
      }
