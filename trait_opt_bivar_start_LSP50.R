...
trait_opt_bivar_start_LSP50 <- function(TLP,
  ...
  for (ss in 1:n_uncer) {
    ...
    if (ss==1) { #First pass always calculates the best estimate
      ...
      mod_Ks_intercept_sample <- Ks_from_P50_LS$mod$intercept_R #Ks_from_P50_LS#_LMA 
     ...
     # mod_Ks_slope_y3_sample <- Ks_from_P50_LS_LMA$mod$slope_R.y3
      ...
      mod_Ks_intercept_sample <- Ks_from_P50_LS$mod$boot.intercept[ss] #Ks_from_P50_LS#_LMA
