...
trait_optim_bivar_start_LSP50 <- function(limitdataranges=T, propagate_uncer=T, nbtstrp=1000, trait_sel= T, n_trait_sel=28, spec_group_sel= 3){
  # ---
  if (propagate_uncer) {
    n_uncer=nbtstrp
  } else {
  ...
  TLP_e <- matrix(NA, nrow= ndata, ncol = n_uncer) #Array now expanded to hold multiple replicate estimates based on regression coefficient uncertainty
  ...
  # Loop over all the combinations of P50 and LS
  # The new estimates of traits use the suffix "_e"
  for (dd in 1:ndata) {
    print(dd)
    
    # Carry out the optimisation - based on P50_e and LS_e as starting points
    
    opt_vals <- trait_opt_bivar_start_LSP50(
  ...
  predicted <- list("TLP_e"=TLP_e,"Ks_e"=Ks_e,"LMA_e"=LMA_e,"WD_e"=WD_e,"slope_e"=slope_e)
  predictors <- list('P50_e' = P50_e,'LS_e'  = LS_e)
