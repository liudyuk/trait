...
trait_opt_bivar_start_LSTLP <- function(P50,
                                        ...
                                        regr_type) {
  # Input (OUTDATED comments [04.07.2022])
  ...
  for (ss in 1:n_uncer) {
   ...
   # LS_e_last = LS_from_LMA$mod$intercept_R  + LS_from_LMA$mod$slope_R.y1*LMA_e_last
    ...
    while (T) {
      ...
      if(use_LMA_from_TLP_LS_WD) { #BE:
        ...
        if(regr_type=='pcr' || regr_type=='plsr'){
          ...
          Ks_e_last   <- scale_traits(Ks_e_last, "Ks_e_last", nlabels, traits_mean, traits_sd)
        ...
        WD_e[ss] = mod_WD_intercept_sample  + mod_WD_slope_y1_sample*P50_e_last  + 
          mod_WD_slope_y2_sample*LMA_e[ss]  +  mod_WD_slope_y3_sample*Ks_e_last
        
        #unscale
        if(regr_type=='pcr' || regr_type=='plsr'){
          P50_e_last <- unscale_traits(P50_e_last, "P50_e_last", nlabels, traits_mean, traits_sd)
          Ks_e_last <- unscale_traits(Ks_e_last, "Ks_e_last", nlabels, traits_mean, traits_sd)
      ...
      if(regr_type=='pcr' || regr_type=='plsr'){
        ...
        LS_e_start   <- unscale_traits(LS_e_start, "LS_e_start", nlabels, traits_mean, traits_sd)
        WD_e[ss]   <- unscale_traits(WD_e[ss], "WD_e_last", nlabels, traits_mean, traits_sd)
      }
       Ks_e[ss] = mod_Ks_intercept_sample + mod_Ks_slope_y1_sample*P50_e[ss] +  mod_Ks_slope_y2_sample* LS_e_start
           +  mod_Ks_slope_y3_sample* WD_e[ss]
      
      
      #unscale
      if(regr_type=='pcr' || regr_type=='plsr'){
        P50_e[ss]  <- unscale_traits(P50_e[ss], "P50_e", nlabels, traits_mean, traits_sd)
        LS_e_start <- unscale_traits(LS_e_start, "LS_e_start", nlabels, traits_mean, traits_sd)
        WD_e[ss]  <- unscale_traits(WD_e[ss], "WD_e_last", nlabels, traits_mean, traits_sd)
      }
    ...
    if(regr_type=='pcr' || regr_type=='plsr'){
      ...
      Ks_e[ss]   <- scale_traits(Ks_e[ss], "Ks_e_start", nlabels, traits_mean, traits_sd)
    }
  ...
  return_vals <- list("P50_e"=P50_e,"TLP_e"=TLP_e_start,"LMA_e"=LMA_e,"WD_e"=WD_e,"slope_e"=slope_e,'LS_e'= LS_e_start,'Ks_e'=Ks_e)
